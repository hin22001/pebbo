...
// Dynamic file naming based on year
...
export function getProgressFileName(year?: number): string {
  return path.join(getBaseDir(year), `image-generation-progress.json`);
}

export function getLogFileName(year?: number): string {
  return path.join(getBaseDir(year), `generate-images.log`);
}

// These will be initialized by the main script once year is known
